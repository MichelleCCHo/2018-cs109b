...
```{r}
### YOUR SOLUTION HERE
model2 <- lm(mpg ~ (cyl + disp + hp)^2, data = mtcars)
model2
```
...
```{r}
### YOUR SOLUTION HERE
model3 <- lm(mpg ~ I(cyl + disp), data = mtcars)
model3
```
...
ggplot(mtc, 
  ...
  geom_point() +
  geom_smooth(mapping = aes(y=pred.mpg), stat="identity")
```
...
```{r}
### YOUR SOLUTION HERE
logit.mod <- glm(am ~ disp, data=mtcars, family=binomial)
logit.mod
```
...
```{r}
### YOUR SOLUTION HERE
logit.mod2 <- glm(am ~ disp + as.factor(gear), data = mtcars, family = binomial)
logit.mod2
```
...
ggplot(mtcars, aes(x = hp, y = mpg)) +
  geom_point() +
  geom_line(aes(y = mpg.ns3, color='blue')) +
  geom_line(aes(y = mpg.ns8, color='red'))
```
...
```{r}
### YOUR SOLUTION HERE
mod.mpg.ns3.int <- lm(mpg ~ ns(hp*am, df = 3), data=mtcars)
summary(mod.mpg.ns3.int)
mtcars$mpg.ns3.int <- predict(mod.mpg.ns3.int)

ggplot(mtcars,
       mapping = aes(x = hp, y = mpg, color = as.factor(am))) +
    geom_point() +
    geom_smooth(mapping = aes(y = mtcars$mpg.ns3.int),
                stat = "identity")
```
...
train <- sample(1:nrow(mtcars), floor(nrow(mtcars) * 0.5))
...
ns.performance<- sapply(dfs, model.performance2, 
                       train = mt.train,
                       test = mt.test,
                       simplify = FALSE) 
