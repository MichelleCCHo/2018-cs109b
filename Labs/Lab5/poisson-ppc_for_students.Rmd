...
# create plotdata
value <- append(y,x)
variable <- append(rep("Our data", N), rep("Poisson data", N))
plotdata <- data.frame(value, variable) 
...
```{r, plot_convergence}
plot(fit, plotfun="trace", pars='mu')
plot(fit, plotfun="trace", pars='sigma')
```
```{r, plot_mu}
color_scheme_set("brightblue") # check out bayesplot::color_scheme_set
mu_draws <- as.matrix(fit, pars = "mu")
mcmc_areas(mu_draws, prob = 0.95) # color 95% credible interval
```
```{r, plot_sigma}
color_scheme_set("brightblue") # check out bayesplot::color_scheme_set
sigma_draws <- as.matrix(fit, pars = "sigma")
mcmc_areas(sigma_draws, prob = 0.95) # color 95% credible interval
```

Compare posterior of lambda to the mean of the data
```{r, print_fit}
means <- c("Posterior mean" = mean(mu_draws), "Data mean" = mean(X))
print(means, digits = 3)
```
